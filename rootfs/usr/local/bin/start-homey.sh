...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202211142203-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  WTFPL
# @@ReadME           :  start-homey.sh --help
# @@Copyright        :  Copyright: (c) 2022 Jason Hempstead, Casjays Developments
# @@Created          :  Monday, Nov 14, 2022 22:03 EST
# @@File             :  start-homey.sh
# @@Description      :  script to start homey
...
__heath_check() {
  ...
  __curl "http://localhost:$SERVICE_PORT/" || healthStatus=$((healthStatus + 1))
...
HOSTNAME="${HOSTNAME:-casjaysdev-homey}"
...
SERVICE_COMMAND="$SERVICE_NAME -b 0.0.0.0:9101 --worker-tmp-dir /dev/shm --workers 2 --threads 50 --log-file - app:app"
...
[ -f "/config/dotenv" ] && ln -sf "/config/dotenv" "/usr/local/share/homey/client/.env"
...
*)
  if __pgrep "$SERVICE_NAME" && [ ! -f "/tmp/$SERVICE_NAME.pid" ]; then
    ...
    echo "$start_message"
    cd "/usr/local/share/homey/api" && __exec_command "$SERVICE_COMMAND" || rm -Rf "/tmp/$SERVICE_NAME.pid"
  fi
  ;;
esac
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
